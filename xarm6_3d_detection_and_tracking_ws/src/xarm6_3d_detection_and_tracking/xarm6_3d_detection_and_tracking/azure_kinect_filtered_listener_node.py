...
class FilteredAzureKinectListenerNode(Node):
    def __init__(self):
        super().__init__('azure_kinect_filtered_listener_node')

        self.bridge = CvBridge()

        # Subscribe to Azure Kinect color image
        self.color_sub = self.create_subscription(
            Image,
            '/rgb/image_raw',  # Azure Kinect RGB topic
            self.color_callback,
            10
        )

        # Subscribe to Azure Kinect depth image aligned to RGB
        self.depth_sub = self.create_subscription(
            Image,
            '/depth_to_rgb/image_raw',  # Azure Kinect aligned depth topic
            self.depth_callback,
            10
        )
